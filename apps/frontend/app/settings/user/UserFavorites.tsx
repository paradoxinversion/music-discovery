...
export const UserFavorites = () => {
  ...
  return (
    <div>
      <h1 className={`${unbounded.className} text-2xl font-bold mb-4}`}>
        User Favorites
      </h1>
      <section id="favorite-tracks" className="mb-6">
        <h2 className="text-xl font-semibold mb-2">Favorite Tracks</h2>
        {data.favorites.favoriteTracks.length === 0 ? (
          <p className="p-2 table-cell">No favorite tracks found.</p>
        ) : (
          <div className="table border border-gray-500 w-full rounded">
            <div className="table-row-group">
              {data.favorites.favoriteTracks.map((track: any) => (
                <div key={track._id} className="table-row hover:bg-gray-900">
                  <div className="p-2 table-cell">{track.title}</div>
                  <div className="p-2 hidden sm:table-cell">
                    {track.artistName}
                  </div>

                  {/* <div className="table-cell border border-gray-500">
                    {track.duration}
                  </div> */}
                  <div
                    className="p-2 table-cell align-middle w-16"
                    onClick={() => {
                      const confirmDelete = confirm(
                        "Are you sure you want to remove this track from your favorites?",
                      );
                      if (confirmDelete) {
                        handleRemoveFavoriteTrack(track._id);
                      }
                    }}
                  >
                    <button className="block m-auto hover:text-red-700">
                      <svg
                  ...
                  <div className="p-2 table-cell m-auto align-middle  w-16">
                    <Link
                      href={`/artist/${track.artistSlug}/track/${track.slug}`}
      ...
      <section id="favorite-artists" className="mb-6">
