...
describe("Create Album", ()=>{
    ...
    it("Throws error if album with same title by same artist exists", async ()=>{
        ...
        await expect(createAlbum(albumData)).rejects.toThrow(`Album with title ${albumData.title} by artist ${albumData.artistId} already exists`);
    });
});
...
describe("Update Album", ()=>{
    it("Updates an album's details", async ()=>{
        ...
        expect(updatedAlbum.releaseDate?.toISOString()).toBe(updateData.releaseDate.toISOString());
    });
    it("Throws error if album to update does not exist", async ()=>{
        ...
        await expect(updateAlbum(fakeAlbumId, updateData)).rejects.toThrow(`Album with ID ${fakeAlbumId} not found`);
    });
});
...
describe("Delete Album", ()=>{
    ...
    it("Throws error if album to delete does not exist", async ()=>{
        const fakeAlbumId = new mongoose.Types.ObjectId().toString();
        await expect(deleteAlbum(fakeAlbumId)).rejects.toThrow(`Album with ID ${fakeAlbumId} not found`);
    });
});