...
describe("Create Track", ()=>{
    ...
    it("Prevents creating duplicate tracks in the same album", async ()=>{
        ...
        await expect(createTrack(trackData)).rejects.toThrow(`Track with title ${trackData.title} in album ${trackData.albumId} already exists`);
    });
});
...
describe("Update Track", ()=>{
    it("Updates track details successfully", async ()=>{
        ...
        const updatedTrack = await updateTrack(track._id.toString(), { title: newTitle });
    ...
    it("Throws error when updating non-existent track", async ()=>{
        const nonExistentId = new mongoose.Types.ObjectId().toString();
        await expect(updateTrack(nonExistentId, { title: "New Title" })).rejects.toThrow(`Track with ID ${nonExistentId} not found`);
    });
});
...
describe("Delete Track", ()=>{
    ...
    it("Throws error when deleting non-existent track", async ()=>{
        const nonExistentId = new mongoose.Types.ObjectId().toString();
        await expect(deleteTrack(nonExistentId)).rejects.toThrow(`Track with ID ${nonExistentId} not found`);
    });
});